...
static uint8_t pub_rec[] = {
0x4A, 0x18, 0x90, 0xE3, 0x0A, 0x58, 0x42, 0x08,
0xDA, 0xD3, 0x83, 0x8D, 0x0C, 0x5C, 0xEC, 0xB1,
0xED, 0x6B, 0x01, 0xD4, 0x88, 0x93, 0xC6, 0x84,
0xC5, 0x99, 0x08, 0xF5, 0xB3, 0x8E, 0x3D, 0x82,
0xBA, 0xE8, 0xFB, 0x2E, 0xF2, 0xDC, 0x08, 0x0F,
0x24, 0x8F, 0x44, 0xFC, 0xEC, 0x45, 0x8B, 0x1B,
0x35, 0xEC, 0xDC, 0x80, 0xF0, 0x0C, 0x95, 0x92,
0x92, 0xCB, 0xE2, 0x1B, 0xE2, 0xFF, 0x2E, 0xA2 };

static uint8_t hash[] = {
0x96, 0x6E, 0xCC, 0xCE, 0x39, 0xE0, 0x5B, 0x54,
0x1B, 0xE5, 0x71, 0x05, 0x5D, 0xC6, 0x3F, 0x7D,
0xF3, 0x77, 0xE7, 0x98, 0x47, 0x16, 0x7E, 0x2B,
0x8E, 0x92, 0xF8, 0x32, 0x7F, 0xAC, 0x38, 0x4D };

static uint8_t ca_key[] = {
0x3B, 0xB8, 0xFF, 0xD1, 0x9B, 0x25, 0xEE, 0x1B,
0xB9, 0x39, 0xCD, 0x49, 0x35, 0xFB, 0xFA, 0x8F,
0xBA, 0xAD, 0xBA, 0x64, 0x84, 0x33, 0x38, 0xA9,
0x55, 0x95, 0xA7, 0x0E, 0xD7, 0x47, 0x9B, 0x70,
0xEB, 0x60, 0xDD, 0xC7, 0x90, 0xE3, 0xCB, 0x05,
0xE8, 0x52, 0x25, 0xF6, 0x36, 0xD8, 0xA7, 0xC2,
0x0D, 0xF3, 0xA8, 0x13, 0x5C, 0x4B, 0x2A, 0xE5,
0x39, 0x63, 0x67, 0xB4, 0xE8, 0x60, 0x77, 0xF8 };

static void v2x_public_key_reconstruction(hsm_hdl_t sv0_sess)
{
	uint8_t out[64];
	uint32_t i;
	hsm_err_t err;
	...
	hsm_op_pub_key_rec_args.pub_rec = pub_rec;
	hsm_op_pub_key_rec_args.pub_rec_size = 2 * 32;

	hsm_op_pub_key_rec_args.hash = hash;
	hsm_op_pub_key_rec_args.hash_size = 32;

	hsm_op_pub_key_rec_args.ca_key = ca_key;
	hsm_op_pub_key_rec_args.ca_key_size = 2 * 32;

	hsm_op_pub_key_rec_args.out_key = out;
	hsm_op_pub_key_rec_args.out_key_size = 2 * 32;

	hsm_op_pub_key_rec_args.key_type = HSM_KEY_TYPE_ECDSA_NIST_P256;
	hsm_op_pub_key_rec_args.flags = 0u;

	err = hsm_pub_key_reconstruction(sv0_sess, &hsm_op_pub_key_rec_args);
...
int main(int argc, char *argv[])
{
	...
	printf("\n---------------------------------------------------\n");
	printf("PUB KEY RECONSTRUCTION test\n");
	printf("---------------------------------------------------\n");
	v2x_public_key_reconstruction(sv0_sess);
