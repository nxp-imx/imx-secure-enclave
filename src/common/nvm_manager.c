...
static uint32_t nvm_storage_import(struct nvm_ctx *nvm_ctx_param, uint8_t *data, uint32_t len)
{
    ...
    do {
        ...
        if (blob_hdr->size + (uint32_t)sizeof(struct nvm_header_s) != len) {
            break;
        }
     
        if (plat_os_abs_crc(data + sizeof(struct nvm_header_s), blob_hdr->size) != blob_hdr->crc) {
            break;
        }
      
        plat_addr = plat_os_abs_data_buf(nvm_ctx_param->phdl, data + sizeof(struct nvm_header_s), blob_hdr->size, DATA_BUF_IS_INPUT);
  
        /* Prepare command message. */
        plat_fill_cmd_msg_hdr(&msg.hdr, SAB_STORAGE_MASTER_IMPORT_REQ, (uint32_t)sizeof(struct sab_cmd_key_store_import_msg), nvm_ctx_param->mu_type);
...
static void nvm_open_session(uint8_t flags)
{
    ...
    do {
        ...
        nvm_ctx.phdl = plat_os_abs_open_mu_channel(nvm_ctx.mu_type, &mu_params);
...
static uint32_t nvm_export_finish_rsp(struct nvm_ctx *nvm_ctx_param, uint32_t error)
{
    ...
    do {
        ...
        plat_fill_rsp_msg_hdr(&resp.hdr, SAB_STORAGE_EXPORT_FINISH_REQ, (uint32_t)sizeof(struct sab_cmd_key_store_export_finish_rsp), nvm_ctx_param->mu_type);
        if (error == 0u) {
        ...
        len = plat_os_abs_send_mu_message(nvm_ctx_param->phdl, (uint32_t *)&resp, (uint32_t)sizeof(struct sab_cmd_key_store_export_finish_rsp));
...
static uint32_t nvm_manager_export_master(struct nvm_ctx *nvm_ctx_param, struct sab_cmd_key_store_export_start_msg *msg, int32_t msg_len)
{
    ...
    do {
        ...
        if (plat_os_abs_storage_write(nvm_ctx_param->phdl, data, data_len) == (int32_t)data_len) {
            /* Success. */
            (void)nvm_export_finish_rsp(nvm_ctx_param, 0u);
        } else {
...
static uint32_t nvm_manager_export_chunk(struct nvm_ctx *nvm_ctx_param, struct sab_cmd_key_store_chunk_export_msg *msg, int32_t msg_len)
{
    ...
    do {
        ...
        if (finish_msg.export_status == SAB_EXPORT_STATUS_SUCCESS) {
            ...
            if (plat_os_abs_storage_write_chunk(nvm_ctx_param->phdl, chunk->data, chunk->len , chunk->blob_id) != (int32_t)(chunk->len)) {
                err = 1;
            }
        }
...
static uint32_t nvm_manager_get_chunk(struct nvm_ctx *nvm_ctx_param, struct sab_cmd_key_store_chunk_get_msg *msg, int32_t msg_len)
{
    ...
    do {
        ...
        if (plat_os_abs_storage_read_chunk(nvm_ctx_param->phdl, (uint8_t *)&nvm_hdr, (uint32_t)sizeof(nvm_hdr), blob_id) == (int32_t)sizeof(nvm_hdr)) {
            data = plat_os_abs_malloc(nvm_hdr.size);
            if (data != NULL) {;
                if (plat_os_abs_storage_read_chunk(nvm_ctx_param->phdl, data, nvm_hdr.size, blob_id) == (int32_t)nvm_hdr.size) {
                    err = 0u;
                }
            }
        }
...
void nvm_manager(uint8_t flags, uint32_t *status)
{
    ...
    do {
         ...
         */
        if (plat_os_abs_storage_read(nvm_ctx.phdl, (uint8_t *)&nvm_hdr, (uint32_t)sizeof(nvm_hdr)) == (int32_t)sizeof(nvm_hdr)) {
            data_len = nvm_hdr.size + (uint32_t)sizeof(nvm_hdr);
            data = plat_os_abs_malloc(data_len);
            if (data != NULL) {
                if (plat_os_abs_storage_read(nvm_ctx.phdl, data, data_len) == (int32_t)data_len) {
