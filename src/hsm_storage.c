...
#define SECO_HSM_NVM_PATH "/dev/seco_hsm_nvm"
...
struct seco_nvm_hdl *seco_nvm_open_session(uint32_t flags, uint8_t *data, uint32_t len)
{
    ...
    do {
        ...
        nvm_hdl->fd = open(SECO_HSM_NVM_PATH, O_RDWR);
        if (nvm_hdl->fd < 0) {
            free(nvm_hdl);
            nvm_hdl = NULL;
            break;
 ...
 * Storage manager
 */
struct hsm_storage_context {
    struct seco_nvm_hdl *nvm_hdl;
    pthread_t tid;
    char *storage_path;
};

static void *seco_nvm_manager_thread(void *arg)
{
    struct hsm_storage_context *ctx = (struct hsm_storage_context *)arg;
    uint8_t *data = NULL;
    uint32_t data_len = 0;
    uint32_t err = 1;
    int32_t fd = -1;
...
#define SECO_NVM_DEFAULT_STORAGE_FILE "/etc/seco_hsm_nvm"

struct hsm_storage_context *hsm_storage_init(void)
{
    struct hsm_storage_context *ctx;
    uint8_t *data = NULL;
    uint32_t data_len = 0;
    uint32_t err = 1;
    int32_t fd = -1;
    ...
    do {
        ctx = malloc(sizeof(struct hsm_storage_context));
        if (!ctx)
            break;
        ...
        ctx->nvm_hdl = seco_nvm_open_session(0/*HSM*/, data, data_len);
        free(data);
        if (!ctx->nvm_hdl) {
            break;
        }
