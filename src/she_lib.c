...
she_err_t she_cmd_export_ram_key(struct she_hdl_s *hdl, uint8_t *m1, uint8_t *m2, uint8_t *m3, uint8_t *m4, uint8_t *m5) {
    ...
    do {
        ...
        /* Build command message. */
        she_fill_cmd_msg_hdr(&cmd.hdr, SAB_SHE_PLAIN_KEY_EXPORT, (uint32_t)sizeof(struct sab_she_plain_key_export_msg));
        cmd.utils_handle = hdl->utils_handle;
        ...
        if ((hdl->cancel != 0u)
            ...
            || (rsp.crc != she_compute_msg_crc((uint32_t*)&rsp, (uint32_t)(sizeof(rsp) - sizeof(uint32_t))))) {
            ret = she_seco_ind_to_she_err_t(rsp.rsp_code);
            she_platform_memset(m1, 0u, SHE_KEY_SIZE);
            she_platform_memset(m2, 0u, 2*SHE_KEY_SIZE);
            she_platform_memset(m3, 0u, SHE_KEY_SIZE);
            she_platform_memset(m4, 0u, 2*SHE_KEY_SIZE);
            she_platform_memset(m5, 0u, SHE_KEY_SIZE);
            hdl->cancel = 0u;
            break;
        ...
        she_platform_memcpy(m1, (uint8_t *)rsp.m1, SHE_KEY_SIZE);
        she_platform_memcpy(m2, (uint8_t *)rsp.m2, 2*SHE_KEY_SIZE);
        she_platform_memcpy(m3, (uint8_t *)rsp.m3, SHE_KEY_SIZE);
        she_platform_memcpy(m4, (uint8_t *)rsp.m4, 2*SHE_KEY_SIZE);
        she_platform_memcpy(m5, (uint8_t *)rsp.m5, SHE_KEY_SIZE);
...
she_err_t she_cmd_get_status(struct she_hdl_s *hdl, uint8_t *sreg) {
    ...
    do {
        /* Build command message. */
        she_fill_cmd_msg_hdr(&cmd.hdr, SAB_SHE_GET_STATUS, (uint32_t)sizeof(struct she_cmd_get_status_msg));
        cmd.she_utils_handle = hdl->utils_handle;
        /* Send the message to Seco. */
        error = she_send_msg_and_get_resp(hdl->phdl,
...
she_err_t she_cmd_get_id(struct she_hdl_s *hdl, uint8_t *challenge, uint8_t *id, uint8_t *sreg, uint8_t *mac) {
	...
	she_platform_memcpy(mac, rsp.mac, SHE_MAC_SIZE);
